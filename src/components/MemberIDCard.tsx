...
export default function MemberIDCard({ member }: MemberIDCardProps) {
  const isApproved = member.status === "APPROVED" || member.status === "ACTIVE" || member.status === "VERIFIED";
  const isRejected = member.status === "REJECTED";
  const memNumber = member.membershipNo || `MEM-2026-${member.id ? member.id.slice(0, 6).toUpperCase() : "7701"}`;

  const defaultPhoto =
    "https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?q=80&w=400&auto=format&fit=crop";

  const photoSrc = member.memberPhoto && member.memberPhoto.trim() !== ""
    ? member.memberPhoto
    : defaultPhoto;
  ...
  if (isRejected) {
    return (
      <div className="w-full max-w-sm mx-auto bg-red-50 rounded-2xl p-6 text-center border border-red-200 shadow-sm space-y-3">
        ...
        <h3 className="text-base font-bold text-red-900">Membership Application Rejected</h3>
        <p className="text-xs text-red-700">
          This membership application was reviewed and rejected by the NGO Admin. No digital membership card has been issued.
        </p>
      </div>
    );
  }
    ...
    <div className={`w-full max-w-sm mx-auto bg-white rounded-2xl shadow-xl overflow-hidden border font-sans transition-all relative ${
      ...
      {!isApproved && (
        <div className="bg-amber-500 text-slate-950 font-black text-[11px] py-1.5 px-3 text-center uppercase tracking-wider flex items-center justify-center gap-1.5 shadow-inner">
          ...
          Membership Pending Admin Review — Card Not Issued
        </div>
      )}
      ...
      <div className="bg-[#031635] text-white p-5 text-center relative border-b-4 border-[#F57C00]">
        <div className="flex items-center justify-center gap-2 mb-1">
          <span className="material-symbols-outlined text-[#F57C00] text-2xl fill">shield</span>
        ...
        <p className="text-[10px] text-[#ffb786] font-bold uppercase tracking-widest">
          {isApproved ? "Official Digital Membership Card" : "Membership Application"}
        </p>
      </div>
      ...
      <div className="p-6 space-y-5 text-center bg-gradient-to-b from-[#f7f9fb] to-white">
        ...
        <div className="relative w-28 h-28 mx-auto">
          ...
          <span
            ...
            title={isApproved ? "Approved Member" : "Pending Verification"}
          >
        ...
        <div className="space-y-1">
          <h3 className="text-xl font-bold text-[#031635] tracking-tight">{member.memberName}</h3>
          <p className="text-xs font-mono font-bold text-[#F57C00]">
            {isApproved ? memNumber : "MEM-2026-PENDING"}
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-2">
          ...
          >
            ...
            {isApproved ? "OFFICIAL NGO MEMBER" : "UNDER ADMIN REVIEW"}
          </span>

          {member.plan?.title && (
            <span className="bg-[#031635] text-white text-[10px] font-bold px-2.5 py-1 rounded-full">
              {member.plan.title}
            </span>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 text-left bg-white p-3.5 rounded-xl border border-[#e0e3e5] text-xs">
          <div>
            ...
            <span className="font-semibold text-slate-800 truncate block">
              {member.validFrom
                ? new Date(member.validFrom).toLocaleDateString("en-IN")
                : new Date().toLocaleDateString("en-IN")}
            </span>
          </div>

          <div>
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Valid Till</span>
          ...
          <div className="col-span-2 border-t border-slate-100 pt-2">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Institutional Privileges</span>
            <span className="font-semibold text-slate-700 block truncate">General Body Voting Rights & 80G Tax Exemption</span>
          </div>
        </div>

        {/* Card Footer Bar */}
        <div className="pt-2 flex justify-between items-center border-t border-slate-100 text-[10px] text-slate-400">
          <span>{isApproved ? "Auth: Ratnakar NGO Board" : "Awaiting Board Verification"}</span>
          <span className="font-mono text-[#031635] font-bold">12A & 80G Certified</span>
        </div>
      </div>
    </div>
  );
